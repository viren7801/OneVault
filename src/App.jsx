...
function App() {
  ...
  return <div className="app-shell"><div className="ambient ambient-one"/><div className="ambient ambient-two"/>
    ...
    {mobileOpen&&<button className="backdrop" onClick={()=>setMobileOpen(false)}/>}<main className="main-area"><header className="topbar"><div className="topbar-left"><button className="icon-btn mobile-menu" onClick={()=>setMobileOpen(true)}><Menu size={20}/></button><div><div className="eyebrow">PRIVATE DASHBOARD</div><h1>{activeModule.label}</h1></div></div><div className="topbar-actions"><span className={`network-status ${isOnline?'online':'offline'}`} title={isOnline?'Online':'Offline'}><span className="network-dot"/><span>{isOnline?'Online':'Offline'}</span></span><UpdateCenter/><button className="command-trigger" onClick={()=>setCommandOpen(true)} title="Command palette"><Search size={15}/><span>Search</span><kbd>⌘K</kbd></button><button className="primary-btn" onClick={()=>active==='pocket'?openTransaction('expense'):active==='reminders'?openReminder():active==='passwords'?openPassword():active==='notes'?openNote():setQuickAdd(true)}><Plus size={17}/> Quick add</button></div></header>
