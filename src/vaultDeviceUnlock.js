...
export const bytesToBase64 = bytesToBase64Url
export const base64ToBytes = base64UrlToBytes

export async function deriveVaultKey(password, salt) {
  const material = await crypto.subtle.importKey('raw', new TextEncoder().encode(password), 'PBKDF2', false, ['deriveKey'])
  return crypto.subtle.deriveKey(
    { name: 'PBKDF2', salt: base64UrlToBytes(salt), iterations: 600000, hash: 'SHA-256' },
    material,
...
export async function encryptVaultJson(value, key) {
  const iv = randomBytes(12)
...
export async function decryptVaultJson(iv, ciphertext, key) {
  const plaintext = await crypto.subtle.decrypt(
